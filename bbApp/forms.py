...
class SaveBloodGroup(forms.ModelForm):
    name = forms.ChoiceField(choices=[('A+','A+'),('A-','A-'),('B+','B+'),('B-','B-'), ('O+','O+'),('AB+','AB+'),('AB-','AB-')], label="Blood Group")
...
class SaveDonation(forms.ModelForm):
    ...
    def clean_donation_volume(self):
        donation_volume = self.cleaned_data['donation_volume']

        try:
            volume = float(donation_volume)  # Attempt to convert to float
            if volume < 0:
                raise forms.ValidationError("Donation volume cannot be negative.")
            return volume  # Return the float value for saving
        except ValueError:
            raise forms.ValidationError("Please enter a valid numerical value for donation volume.")
...
class SaveRequest(forms.ModelForm):
    ...
    def clean_blood_group(self):
        bgID = self.cleaned_data['blood_group']
        bgID = bgID if bgID.isnumeric() and bgID > 0 else 0
        try:
            blood_group = models.Blood_Group.objects.get(id=bgID)
            return blood_group
        except:
    ...
    def clean_donation_volume(self):
        requested_volume = self.cleaned_data['volume']

        try:
            volume = float(requested_volume)  # Attempt to convert to float
            if volume < 0:
                raise forms.ValidationError("Requested volume cannot be negative.")
            return volume  # Return the float value for saving
        except ValueError:
            raise forms.ValidationError("Please enter a valid numerical value for donation volume.")
